...
import {
	AdminFilteredEntries,
	PublicEntry,
	QueriedEntries,
} from "../models/response/entries.response.js";
...
import {
	...
	FilterFull,
	FilterQuery,
} from "../models/request/entries.request.js";
import validate, {
	validateFilterQuery,
	validateId,
	validateOptional,
...
@Controller("entries")
@ClassMiddleware(csrfMiddleware)
export default class EntriesController {
	...
	async authorizedGetUnapproved(
		req: IRequest<{}, { page: number }>,
		res: IResponse<QueriedEntries>
	) {
		res.send(
			await EntryService.getUnapproved(
	...
	async adminGetFullFilteredEntries(
		req: IRequest<FilterFull>,
		res: IResponse<AdminFilteredEntries>
	) {
	...
	async getSingleEntry(
		req: IRequest<{}, {}, ObjectId>,
		res: IResponse<PublicEntry>
	) {
	...
	async blockEntry(req: IRequest<{}, {}, ObjectId>, res: IResponse) {
		...
		let updated = await EntryService.update(entry, {
			blocked: true,
	...
	async adminUpdateEntry(
		req: IRequest<EditEntry, {}, ObjectId>,
		res: IResponse
	) {
		let entry = await Database.getEntryById(req.params.id);

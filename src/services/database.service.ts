...
import {
	CollectionMeta,
	EntriesCollectionMeta,
	CollectionMetaUpdateType,
} from "../models/database/collectionMeta.model.js";
...
export function connect() {
	...
	client.connect((err) => {
		...
		Promise.all([
			...
			db.collection<DatabaseEntry<"in">>("entries").createIndex({
				name: "text",
				academicTitle: "text",
				firstName: "text",
				lastName: "text",
			...
			db
				.collection<GeoData>("geodata")
				.createIndex({ name: "text", plz: "text", ascii: "text" }),
			db
				.collection<EntriesCollectionMeta<"in">>("meta")
				.createIndex({ about: "text" }),
		]).then(() => {
...
): Promise<DatabaseEntry<"out"> | null> {
	return (await db.collection<DatabaseEntry<"in">>("entries").findOne({
		_id: new MongoDB.ObjectId(entryId),
	})) as unknown as DatabaseEntry<"out">;
}
...
export async function findEntries(
	query: MongoDB.Filter<DatabaseEntry<"in">>,
	page: number
): Promise<PublicEntry[]> {
 ...
 * @returns Array with entry objects
 */
export async function findEntriesAtLocation(
	locaction: GeoJsonPoint,
	query: MongoDB.Filter<DatabaseEntry<"in">> = {},
	page = 0
): Promise<PublicEntry[]> {
	let limit = Config.config.mongodb.itemsPerPage;
 ...
 */
export async function findEntriesRaw(
	pipeline: object[] | undefined
): Promise<DatabaseEntry<"out">[]> {
	return (await db
		.collection<DatabaseEntry<"in">>("entries")
		.aggregate(pipeline)
...
export async function updateEntry(
	entry: DatabaseEntry<"out">,
	updater: Partial<DatabaseEntry<"in">>,
	additionalUpdater: MongoDB.UpdateFilter<DatabaseEntry<"in">> = {}
): Promise<boolean> {
	let res = await db
		.collection<DatabaseEntry<"in">>("entries")
		.updateOne(
			{ _id: new MongoDB.ObjectId(entry._id) },
			{ $set: updater, ...additionalUpdater }
		);
...
export async function exportEntries(): Promise<string | false> {
	...
	let meta = (await db.collection<CollectionMeta<"in">>("meta").findOne({
		about: "entries",
	})) as unknown as EntriesCollectionMeta<"out">;
	let backupPath = path.join(
		Config.config.mongodb.backupFolder,
		meta.lastExportTimestamp.toString(),
		"/entries.json"
	);
	let success = true;
	...
	if (
		meta.lastChangeTimestamp > meta.lastExportTimestamp ||
		...
		if (success) {
			updateEntriesCollectionMeta(
				CollectionMetaUpdateType.Exported,
				timestamp
			);
		}
	}
 ...
 * @param timestamp
 */
async function updateEntriesCollectionMeta(
	type = CollectionMetaUpdateType.Changed,
	timestamp: number = Date.now()
): Promise<void> {
	// Check if entry meta exists
	let meta = await db
		.collection<CollectionMeta<"out">>("meta")
		.findOne({ about: "entries" });
	...
	if (!meta) {
		const entriesMeta: EntriesCollectionMeta<"in"> = {
			...
			lastExportTimestamp:
				type === CollectionMetaUpdateType.Exported ? timestamp : 0,
		};

		await db
			.collection<CollectionMeta<"in">>("meta")
			.insertOne(entriesMeta);
	} else {
